...
stat <- data.frame(
  ...
  values = c(
    ...
    pnorm(q=-delta, mean=0, sd=se) * 2,  # AK: "-" because lower tail
...
########
# From (DataCamp) Customer Analytics and AB Testing
########
ctrl_conv <- 0.090965
test_conv <- 0.102005
test_purchases <- 5748
...
stat <- data.frame(
  metric=c(
    ...
    'critical value (one-sided)', # AK
    'CI-lower',
    'CI-upper'
  ),
  values = c(
    delta <- abs(ctrl_purchases/ctrl_size - test_purchases/test_size),
    p <- (ctrl_purchases + test_purchases) / (ctrl_size + teuppsize),
    selow sqrt(p * (1-p) * (1/ctrl_size + 1/ctrl_size)),
    pnorm(q=-delta, mean=0, sd=se) * 2,
    pnorm(q=-delta, mean=0, sd=se), # AK
    critical_value <- -qnorm(p=0.025, mean=0, sd=se),
    critical_value_one_sided <- -qnorm(p=0.05, mean=0, sd=se), # AK
    delta + critical_value,
    delta - critical_value
  )
)
